<template>
  <div class="min-h-screen text-slate-100 space-y-8">
    ...
    <div class="rounded-2xl bg-slate-900/70 border border-white/10 shadow-2xl p-6">
      ...
      <p class="text-slate-300 mb-4">ส่งการแจ้งเตือนให้กับผู้ใช้ และจัดการเทมเพลต</p>

      <div class="grid grid-cols-2 gap-4">
        <div class="border rounded-lg p-4">
          <h3 class="font-semibold mb-2">การแจ้งเตือนรายบุคคล</h3>
          <p class="text-sm text-slate-300 mb-2">ส่งการแจ้งเตือนถึงผู้ใช้เฉพาะราย</p>
        ...
        <div class="border rounded-lg p-4">
          ...
          <p class="text-sm text-slate-300 mb-2">ส่งการแจ้งเตือนถึงผู้ใช้ทุกคน</p>
      ...
      <div class="bg-white rounded-lg p-8 max-w-md w-full">
        ...
        <form @submit.prevent="sendNotification" class="space-y-4">
          <div>
            ...
            <input v-model="form.title" type="text" required class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-amber-300/60 focus:ring-2 focus:ring-amber-400/60 text-slate-100" />
          </div>
          <div>
            <label class="block text-sm font-medium mb-1">ข้อความ *</label>
            <textarea v-model="form.message" required rows="3" class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-amber-300/60 focus:ring-2 focus:ring-amber-400/60 text-slate-100"></textarea>
          </div>
    ...
    <div v-if="showBroadcastModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-8 max-w-md w-full">
        ...
        <form @submit.prevent="sendBroadcast" class="space-y-4">
          <div>
            <label class="block text-sm font-medium mb-1">หัวข้อ *</label>
            <input v-model="broadcastForm.title" type="text" required class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-amber-300/60 focus:ring-2 focus:ring-amber-400/60 text-slate-100" />
          </div>
          <div>
            <label class="block text-sm font-medium mb-1">ข้อความ *</label>
            <textarea v-model="broadcastForm.message" required rows="3" class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-amber-300/60 focus:ring-2 focus:ring-amber-400/60 text-slate-100"></textarea>
          </div>
          <div class="flex gap-4">
